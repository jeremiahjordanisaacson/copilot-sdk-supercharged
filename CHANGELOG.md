...
## [v2.5.3](https://github.com/jeremiahjordanisaacson/copilot-sdk-supercharged/releases/tag/v2.5.3) (2026-08-22)

Maintenance, security, and documentation release. No public API changes.

### Fixes

- **Python (PyPI packaging)** — corrected the distribution name to `copilot-sdk-supercharged` (in `python/pyproject.toml` and the `importlib.metadata` version lookup in `python/copilot/__init__.py`) so the Python SDK publishes under the same name used on every other registry, instead of a separate unowned PyPI project.

### Security

- **Python codegen** — fixed a High-severity ReDoS (super-linear regex backtracking) in the class-block matcher in `scripts/codegen/python.ts` by tightening `[^\n]*` to `[^\r\n]*`. Behavior-preserving; resolves the open CodeQL alert.

### CI / Publishing

- Moved the npm, PyPI, and pub.dev publish workflows to OIDC trusted publishing, and added `workflow_dispatch` triggers plus a manifest-version fallback so a release can be safely re-run without re-tagging.

### Docs

- Backfilled a source-grounded **Recent Features (v2.4–v2.5)** section into all 40 SDK READMEs, documenting features already implemented in code (tool search, session rewind, content exclusion, additional directories, disabled MCP servers, session hooks, BYOK bearer-token provider, and more). Every documented symbol was grep-verified against actual SDK source.

## [v2.5.2](https://github.com/jeremiahjordanisaacson/copilot-sdk-supercharged/releases/tag/v2.5.2) (2026-08-21)
...
Maintenance release hardening the additional-language end-to-end test suites. No public API changes.

- **Perl** — implemented the `sessionFs.*` provider callbacks (`readFile`, `writeFile`, `appendFile`, `exists`, `stat`, `mkdir`, `readdir`, `readdirWithTypes`, `rm`, `rename`) so configuring a `sessionFs` provider no longer fails session creation. Void operations now reply with JSON `null` (via a JSON-RPC null-result sentinel), matching what the CLI expects.
- **Scala** — reframed the JSON-RPC transport to read message bodies by byte length instead of UTF-8 characters, eliminating message desync and the resulting `session.idle` timeouts; fixed a `ping()` re-entrancy deadlock during `start()`; made `ModelCapabilities` fields optional so model listings decode.
- **Lua** — corrected process-timeout placement so the CLI child process is wrapped correctly, and removed an unsupported custom-agent reference from the E2E config test.
- Test corrections across Perl (documented `SessionConfig`/`define_tool` API, `sessionId`/`get_state` accessors) and Scala (session-list assertion, filesystem `conventions`).

## [v2.5.0](https://github.com/jeremiahjordanisaacson/copilot-sdk-supercharged/releases/tag/v2.5.0) (2026-08-20)

### Upstream sync: 210 commits from `github/copilot-sdk`

Synced the full upstream history and ported every new user-facing feature to all 40 SDKs.
New capabilities:

- **Session rewind** — restore a session to an earlier point from its file-change captures (`rewind`). ([#2321](https://github.com/github/copilot-sdk/pull/2321))
- **Additional session directories** — expose extra working directories to a session (`additionalDirectories`). ([#2180](https://github.com/github/copilot-sdk/pull/2180))
- **Disabled MCP servers** — turn off configured MCP servers per session (`disabledMcpServers`). ([#2260](https://github.com/github/copilot-sdk/pull/2260))
- **GitHub MCP tool config** — configure the built-in GitHub MCP tools (`githubMcpToolConfig`). ([#2112](https://github.com/github/copilot-sdk/pull/2112))
- **Canvas provider** — register a session-level canvas provider (`canvasProvider`). ([#1847](https://github.com/github/copilot-sdk/pull/1847))
- **Custom agents local-only** — restrict custom-agent discovery to the workspace (`customAgentsLocalOnly`). ([#1899](https://github.com/github/copilot-sdk/pull/1899))
- **`userPromptTransformed` hook** — observe the transformed user prompt. ([#2254](https://github.com/github/copilot-sdk/pull/2254))
- **Permission decision context** — forward opaque `decisionContext` on permission replies. ([#2294](https://github.com/github/copilot-sdk/pull/2294))
- **Built-in plugin directories** — load built-in plugins from extra dirs (`builtinPluginDirectories`). ([#2330](https://github.com/github/copilot-sdk/pull/2330))
- **Agent Factories authoring surface** — declare a factory's arguments with `argsSchema`. ([#2114](https://github.com/github/copilot-sdk/pull/2114), [#2315](https://github.com/github/copilot-sdk/pull/2315))
- **`reasoningEffort: max`** — highest reasoning-effort tier. ([#2228](https://github.com/github/copilot-sdk/pull/2228))
- **Tool search configuration** — scale to large tool sets (`toolSearch`). ([#1933](https://github.com/github/copilot-sdk/pull/1933))
- **In-process (FFI) transport** — run the runtime in-process across Node, Python, Go, and Rust (`inProcess`). ([#1953](https://github.com/github/copilot-sdk/pull/1953), [#1975](https://github.com/github/copilot-sdk/pull/1975), [#1976](https://github.com/github/copilot-sdk/pull/1976), [#1915](https://github.com/github/copilot-sdk/pull/1915))
- **Experimental mode** — opt into experimental runtime features (`experimentalMode`). ([#1600](https://github.com/github/copilot-sdk/pull/1600))
- **Content exclusion** — respect repository content-exclusion rules (`contentExclusion`).
- Plus managed permission settings, per-agent reasoning effort, opaque tool `metadata`, and startup-timing breakdowns tracked upstream in the core SDKs.

All 15 new features are enforced across the 36 additional SDKs by
`scripts/verify-sdk-coverage.mjs`. Core SDKs (Node, Python, Go, .NET) track upstream directly;
Java and Rust remain first-class supercharged ports.

## [v2.4.0](https://github.com/jeremiahjordanisaacson/copilot-sdk-supercharged/releases/tag/v2.4.0) (2026-07-07)

### Upstream sync: 369 commits from `github/copilot-sdk` (through java/v1.0.5-01)

Synced the full upstream history from 2026-05-05 to 2026-07-07 and ported every new
user-facing feature to all 40 SDKs. New capabilities:

- **MCP OAuth host token handlers** — register an `onMcpAuthRequest` callback to answer
  `401 WWW-Authenticate` challenges from MCP servers (initial, refresh, reauth, upscope). ([#1669](https://github.com/github/copilot-sdk/pull/1669))
- **BYOK bearer-token provider** — `bearerTokenProvider` callback (renamed from `getBearerToken`)
  with per-session `sessionId` scoping for Managed-Identity-style token resolution. ([#1748](https://github.com/github/copilot-sdk/pull/1748), [#1796](https://github.com/github/copilot-sdk/pull/1796))
- **New session options** — `enableCitations`, `excludedBuiltinAgents`, and `sessionLimits`
  (per-session AI-credit budget via `maxAiCredits`). ([#1865](https://github.com/github/copilot-sdk/pull/1865))
- **Opt-in session memory** — `memory: { enabled: true }` (`MemoryConfiguration`). ([#1617](https://github.com/github/copilot-sdk/pull/1617))
- **WebSocket transport** — `enableWebSocketResponses` and `provider.transport`. ([#1711](https://github.com/github/copilot-sdk/pull/1711))
- **Tool `defer` loading** — eager (`never`) vs. lazy (`auto`) tool surfacing. ([#1632](https://github.com/github/copilot-sdk/pull/1632))
- **System-message `preamble` section and `preserve` action.** ([#1713](https://github.com/github/copilot-sdk/pull/1713))
- **Message options** — `agentMode` and `displayPrompt` on `send`. ([#1438](https://github.com/github/copilot-sdk/pull/1438), [#1470](https://github.com/github/copilot-sdk/pull/1470))
- **Hooks** — post-tool-use failure hooks (`onPostToolUse`) and `preMcpToolCall`. ([#1421](https://github.com/github/copilot-sdk/pull/1421), [#1366](https://github.com/github/copilot-sdk/pull/1366))
- **HTTP request handler** — intercept/mutate/forward outbound LLM inference requests. ([#1775](https://github.com/github/copilot-sdk/pull/1775))
- **OTLP telemetry protocol** (`otlpProtocol`), **experiment assignments** (`expAssignments`),
  and **GitHub-anchored attachment variants** (`GitHubCommit`, `GitHubRelease`, …). ([#1648](https://github.com/github/copilot-sdk/pull/1648), [#1750](https://github.com/github/copilot-sdk/pull/1750), [#1823](https://github.com/github/copilot-sdk/pull/1823))

All 18 new features are enforced across the 36 additional SDKs by
`scripts/verify-sdk-coverage.mjs`. Core SDKs (Node, Python, Go, .NET) track upstream directly.
...
Published the Java SDK to Maven Central under `io.github.jeremiahjordanisaacson:copilot-sdk-supercharged`. Added publish workflows for 6 new registries: Maven Central, Packagist, pub.dev, Hackage, CPAN, and CRAN.
...
Replaced the anonymous `new { sessionId }` with a proper `SetForegroundSessionRequest` record type in `SetForegroundSessionIdAsync()`. This fixes the runtime `System.Text.Json` metadata failure when using source-generated serialization (required for AOT/trimming). ([upstream #1144](https://github.com/github/copilot-sdk/pull/1144))

## [v2.0.0](https://github.com/jeremiahjordanisaacson/copilot-sdk-supercharged/releases/tag/v2.0.0) (2026-04-26)
