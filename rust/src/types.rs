...
///
/// Controls whether the agent responds with text, an image, or a JSON object.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
#[non_exhaustive]
pub enum ResponseFormat {
    ...
    Text,
    /// Image response — use with [`ImageOptions`] to control size/quality/style.
    Image,
    /// JSON object response.
    JsonObject,
}

/// Options for image generation when [`ResponseFormat::Image`] is used.
///
/// All fields are optional. The agent uses sensible defaults when omitted.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[non_exhaustive]
...
pub struct MessageOptions {
    ...
    /// Desired response format for this turn.
    ///
    /// `"text"` (default), `"image"`, or `"json_object"`.
    pub response_format: Option<ResponseFormat>,
    /// Options for image generation (only used when `response_format` is
...
impl MessageOptions {
    ...
    /// Set the desired response format for this turn.
    pub fn with_response_format(mut self, format: ResponseFormat) -> Self {
        self.response_format = Some(format);
        self
    }

    /// Set image generation options (used when `response_format` is `Image`).
